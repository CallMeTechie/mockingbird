...
printf '$primary: #314BD3\n--sassy: #123456\n' > "$PROJ/client/styles/_colors.sass"
printf 'token_definitions: [client/styles/brand.scss, client/styles/_colors.sass]\n' >> "$PROJ/docs/design/manifest.yaml"
printf '.w { color: #FFFFFF; }\n.s { color: #aabbcc; }\n.p { color: #314bd3; }\n' > "$PROJ/client/styles/match.css"
check "raw value with exactly one token -> token named" "1" "$("$SCOPE" --tokens --root "$PROJ" | grep -cF 'rogue.sass:[0-9]*:--rogue-match:')"
...
check "raw value with no token -> -" "1" "$("$SCOPE" --tokens --root "$PROJ" | grep -cF 'Inline.tsx:1:-:')"
