...
seam_line A "$SANDBOX/src/Real.tsx:1" - - - "$SANDBOX/src/Real.tsx:1" x violated > "$SANDBOX/s.txt"
check "absolute path UNDER the root is accepted as evidence" "0" "$(mb_check_seam "$SANDBOX/s.txt" "$SANDBOX" | grep -c 'bad file:line')"
